...
public class CarouselBaseView extends FrameLayout {
    ...
    protected IndicatorBaseView mIndicator;
    ...
    public void load(){
        ...
        params.height = (int) mIndicator.indicatorHeight;
    ...
    private void refreshIndicator(){
        if (mPagerAdapter.getCount() <= 1) {
            ...
            params.width = (int) (mIndicator.indicatorWidth * (mPagerAdapter.getCount() - 2) +
                    mIndicator.indicatorSpacing  * (mPagerAdapter.getCount() - 3));
            mIndicator.setLayoutParams(params);
        }
        mIndicator.setCount(mPagerAdapter.getCount());
    }
    ...
    private ViewPager.OnPageChangeListener mPageLoopListener = new ViewPager.OnPageChangeListener() {
        ...
        public void onPageSelected(int position) {
            ...
            if (mIndicator!=null && mPagerAdapter.getCount()!=0) {
                mIndicator.indicatedPosition = position%mPagerAdapter.getCount();
                //仅刷新状态不刷新布局
                mIndicator.refreshView();
            }
        }
    ...
    public CarouselBaseView size(float width, float height, float spacing){
        mIndicator.indicatorWidth = dp2Px(width);
        mIndicator.indicatorHeight = dp2Px(height);
        mIndicator.indicatorSpacing = dp2Px(spacing);
        return this;
    }
    ...
    public CarouselBaseView indicatorBg(int select, int unSelect){
        mIndicator.setBgResource(select,unSelect);
        return this;
    }
